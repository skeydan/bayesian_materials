...
<div id="exercise">
**Exercise:** Examine the residuals of this model.  Is the assumption of normally distributed residuals reasonable? #remove#yes
</div>
```{r log-resid}
# type your code for the Exercise here, and Knit
...
df <- 993
c <- 0.975
lower <- b - qt(c, df=df) * s
lower
upper <- b + qt(c, df=df) * s
upper
```
...
bma_wage_red = bas.lm(lwage ~ . -wage, data = wage_no_na,
                   prior = "BIC", 
                   modelprior = uniform())
bma_wage_red
